...
module.exports = {
  ...
  async execute(message, args, user) {
    ...
    const minW = minCfg ? Number(minCfg.value) || 1000 : 1000;
    ...
    try {
      ...
      const result = await ApironeAPI.createWithdrawal('ltc', cryptoAmount, address).catch(e => null);
      if (result && result.id) {
        await Transaction.findOneAndUpdate({ transactionId: txId }, { status: 'completed', cryptoHash: result.id });
      } else if (result && result.transfer_key) {
        await Transaction.findOneAndUpdate({ transactionId: txId }, { status: 'completed' });
      }

      const embed = EmbedHelper.createDefault()
        .setTitle('📤 Withdrawal ' + config.emojis.litecoin)
        .setDescription('Successfully submitted **' + pointsAmount + '** points')
        .addFields(
        ...
        .setFooter({ text: 'Flipbets • Withdrawal' })
        .setTimestamp();
